...
const translations = {
  en: {
    ...
    demoPlay: "Play demo",
    outOfBounds: "Out of bounds! Stay on the letter.",
    boundaryWarning: "Boundary Warning",
...
// ─── DETECTION POINTS GENERATOR ─────────────────────────────────
// Generates boundary detection points around the letter shape
// Based on the image annotation showing numbered stroke detection arrows
function generateDetectionPoints(letter, canvasW, canvasH) {
  const cx = canvasW / 2;
  const cy = canvasH / 2 + canvasH * 0.04;
  const fontSize = Math.round(canvasH * 0.65);
  
  // Generate a grid of detection points around the letter boundary
  // Points are placed in a pattern similar to the numbered arrows in the image
  // The image shows 7 detection points (1-7) arranged around the letter's strokes
  
  const points = [];
  const letterBoundRadius = fontSize * 0.38;
  
  // Main detection ring - 8 cardinal + diagonal points
  const angles = [0, 45, 90, 135, 180, 225, 270, 315];
  angles.forEach((angleDeg, i) => {
    const rad = (angleDeg * Math.PI) / 180;
    points.push({
      id: i + 1,
      x: cx + Math.cos(rad) * letterBoundRadius,
      y: cy + Math.sin(rad) * letterBoundRadius * 0.85,
      radius: 18,
      active: false,
      warning: false,
    });
  ...
  // Inner detection points (closer to letter center strokes)
  const innerAngles = [30, 150, 210, 330];
  innerAngles.forEach((angleDeg, i) => {
    const rad = (angleDeg * Math.PI) / 180;
    points.push({
      id: points.length + 1,
      x: cx + Math.cos(rad) * letterBoundRadius * 0.55,
      y: cy + Math.sin(rad) * letterBoundRadius * 0.55,
      radius: 14,
      active: false,
      warning: false,
    });
...
// Check if a point is outside the letter boundary
function isOutsideBoundary(px, py, guideCanvas) {
  ...
  try {
    ...
    // Check a small region around the draw point
    ...
    // If less than 5% of surrounding area is letter, we're outside
    return total > 0 && (letterPixels / total) < 0.05;
  } catch {
    return false;
  }
}
...
function playWarningSound() {
  try {
    ...
    // Short sharp beep - boundary warning
...
function DetectionPointsOverlay({ points, canvasW, canvasH, showPoints, activeWarning }) {
  if (!showPoints || !points.length) return null;
  
  // Scale for display
  const scaleX = 100; // percentage-based
  const scaleY = 100;
    ...
    >
      {points.map((pt, i) => (
        <g key={pt.id}>
          {/* Outer ring */}
          <circle
            cx={pt.x} cy={pt.y} r={pt.radius + 4}
            fill="none"
            stroke={pt.warning ? '#e02424' : 'rgba(17,17,17,0.12)'}
            strokeWidth={pt.warning ? 2 : 1}
            strokeDasharray={pt.warning ? 'none' : '3 3'}
          />
          {/* Inner dot */}
          <circle
            cx={pt.x} cy={pt.y} r={6}
            fill={pt.warning ? '#e02424' : 'rgba(17,17,17,0.2)'}
            opacity={pt.warning ? 1 : 0.7}
          />
          {/* Number label */}
          <text
            x={pt.x} y={pt.y - pt.radius - 6}
            textAnchor="middle"
            fontSize="10"
            fontFamily="DM Sans, sans-serif"
            fontWeight="700"
            fill={pt.warning ? '#e02424' : 'rgba(17,17,17,0.4)'}
          >
            {pt.id}
          </text>
          {/* Warning pulse ring */}
          {pt.warning && (
            <circle
              cx={pt.x} cy={pt.y} r={pt.radius + 12}
              fill="none"
              stroke="#e02424"
              strokeWidth="1.5"
              opacity="0.5"
      ...
      {/* Boundary polygon connecting all outer points */}
      {points.length >= 3 && (
        <polygon
          points={points.map(p => `${p.x},${p.y}`).join(' ')}
          fill="none"
          stroke={activeWarning ? 'rgba(224,36,36,0.3)' : 'rgba(17,17,17,0.06)'}
          strokeWidth={activeWarning ? 2 : 1}
          strokeDasharray="4 4"
        />
      )}
    </svg>
...
export default function LetterTracingPage({ lang = 'en' }) {
  ...
  const warningCoolRef = useRef(0); // timestamp for cooldown
  ...
  const buildGuideCanvas = useCallback((letter, w, h) => {
    ...
    // Generate detection points after building guide canvas
    const pts = generateDetectionPoints(letter, w, h);
    setDetectionPoints(pts);
  }, []);
  ...
  // ── Boundary check & warning trigger ──
  const checkBoundaryAndWarn = useCallback((px, py) => {
    const now = Date.now();
    // 600ms cooldown to avoid spam
    if (now - warningCoolRef.current < 600) return;

    const outside = isOutsideBoundary(px, py, guideRef.current);
    if (outside) {
      ...
      // Update detection points to show which are near the violation
      setDetectionPoints(prev => prev.map(pt => {
        const dist = Math.hypot(pt.x - px, pt.y - py);
        return { ...pt, warning: dist < 80 };
      }));
      ...
      // Auto-clear warning flash after 500ms
      setTimeout(() => {
        setBoundaryWarning(false);
        setDetectionPoints(prev => prev.map(pt => ({ ...pt, warning: false })));
      }, 500);
    }
  }, []);
  ...
  const draw = (e) => {
    ...
    // Real-time boundary check every draw event
    checkBoundaryAndWarn(x, y);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: 'DM Sans, sans-serif', color: '#111', paddingTop: 80 }}>
      <style>{`
        ...
        .boundary-active { animation: warningPulse 0.3s ease; }
      ...
      <div style={{ position: 'sticky', top: 80, zIndex: 40, background: '#fff', borderBottom: '0.5px solid #e5e7eb', padding: '10px 24px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 28, flexShrink: 0 }}>
            ...
            {/* Boundary warning counter in top bar */}
            {warningCount > 0 && (
              <div style={{ textAlign: 'right' }}>
                <div className="font-display" style={{ fontSize: 18, fontWeight: 800, lineHeight: 1.1, color: '#dc2626' }}>
                  {warningCount}
                </div>
                <div className="font-body" style={{ fontSize: 10, color: '#dc2626', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Warnings</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 24px', display: 'grid', gridTemplateColumns: '220px 1fr 240px', gap: 24 }}>
          ...
          <div style={{ height: 'calc(100vh - 280px)', overflowY: 'auto', paddingRight: 4 }}>
            ...
            {activePanel === 'guide' && (
              <div style={{ paddingTop: 4 }}>
                ...
                {/* Detection info in guide panel */}
                <div style={{ borderTop: '0.5px solid #e5e7eb', paddingTop: 16, marginTop: 16 }}>
                  <div className="font-body" style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.14em', color: '#aaa', marginBottom: 8 }}>
                    Boundary detection
                  </div>
                  <p className="font-body" style={{ fontSize: 12, color: '#555', lineHeight: 1.6, marginBottom: 10 }}>
                    {detectionPoints.length} detection points surround this letter. When your stroke goes outside the boundary, you will hear a warning beep and feel a vibration.
                  </p>
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 10, height: 10, borderRadius: '50%', background: 'rgba(17,17,17,0.2)', border: '1px dashed #888' }} />
                      <span className="font-body" style={{ fontSize: 11, color: '#888' }}>Normal</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#dc2626' }} />
                      <span className="font-body" style={{ fontSize: 11, color: '#888' }}>Warning</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <main style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <button onClick={() => setShowDetectionPoints(v => !v)} style={{
                display: 'flex', alignItems: 'center', gap: 6,
                padding: '7px 14px', borderRadius: 8,
                border: `0.5px solid ${showDetectionPoints ? '#dc2626' : '#e5e7eb'}`,
                background: showDetectionPoints ? '#fef2f2' : '#fff',
                color: showDetectionPoints ? '#dc2626' : '#888',
                fontFamily: 'DM Sans, sans-serif', fontSize: 12, cursor: 'pointer',
              }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <circle cx="12" cy="12" r="3"/><circle cx="12" cy="12" r="9" strokeDasharray="3 3"/>
                </svg>
                {showDetectionPoints ? 'Points on' : 'Points off'}
              </button>
          ...
          {boundaryWarning && (
            <div style={{
              ...
              <span className="font-body" style={{ fontSize: 13, fontWeight: 500, color: '#dc2626' }}>
          ...
          {/* Canvas card */}
          <div
            className={`hover-lift${boundaryWarning ? ' boundary-active' : ''}`}
            style={{
              background: '#fff',
          ...
          >
            <div style={{ padding: '10px 16px', borderBottom: '0.5px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span className="font-body" style={{ fontSize: 11, color: '#bbb', marginLeft: 4 }}>
                — {detectionPoints.length} detection points active
              </span>
              {celebrating && (
              ...
              {warningCount > 0 && !celebrating && (
                <span className="warning-badge" style={{ marginLeft: 'auto' }}>
          ...
          <div style={{ background: '#fafafa', border: '0.5px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '10px 16px', borderBottom: '0.5px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span className="font-body" style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#888' }}>Voice guidance &amp; boundary alerts</span>
              </div>
            ...
            <div style={{ padding: '12px 16px', maxHeight: 120, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
              {alertLog.length === 0 ? (
                <p className="font-body" style={{ fontSize: 12, color: '#bbb', textAlign: 'center', padding: '8px 0' }}>
                  Voice guidance and boundary warnings will appear here.
                </p>
              ) : (
                alertLog.slice().reverse().map((a, i) => (
                  <div key={i} className="log-entry" style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    ...
                    <p className="font-body" style={{
                      fontSize: 12,
                      color: a.type === 'warning' ? '#dc2626' : '#555',
                      margin: 0, lineHeight: 1.5,
                      fontWeight: a.type === 'warning' ? 500 : 400,
                    }}>{a.text}</p>
                  </div>
                ))
              )}
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ border: '0.5px solid #e5e7eb', borderRadius: 16, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px 20px', background: '#fff' }}>
              {[
                ...
                { label: 'Detect pts', value: `${detectionPoints.length}` },
              ].map(({ label, value }) => (
          ...
          {/* Boundary warning stats */}
          <div style={{
            border: warningCount > 0 ? '1px solid #fca5a5' : '0.5px solid #e5e7eb',
            borderRadius: 16, padding: '16px 20px',
            background: warningCount > 0 ? '#fef2f2' : '#fafafa',
            transition: 'all 0.3s',
          }}>
            <div className="font-body" style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.14em', color: warningCount > 0 ? '#dc2626' : '#aaa', marginBottom: 12 }}>
              Boundary status
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div style={{ padding: '12px 14px', borderRadius: 10, background: warningCount > 0 ? '#fee2e2' : '#fff', border: '0.5px solid #e5e7eb' }}>
                <div className="font-display" style={{ fontSize: 22, fontWeight: 800, color: warningCount > 0 ? '#dc2626' : '#111', lineHeight: 1.1 }}>
                  {warningCount}
                </div>
                <div className="font-body" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: warningCount > 0 ? '#dc2626' : '#aaa', marginTop: 3 }}>
                  Warnings
                </div>
              </div>
              <div style={{ padding: '12px 14px', borderRadius: 10, background: '#fff', border: '0.5px solid #e5e7eb' }}>
                <div className="font-display" style={{ fontSize: 22, fontWeight: 800, color: '#111', lineHeight: 1.1 }}>
                  {detectionPoints.length}
                </div>
                <div className="font-body" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#aaa', marginTop: 3 }}>
                  Points
                </div>
              </div>
            </div>
            {warningCount > 0 && (
              <p className="font-body" style={{ fontSize: 12, color: '#dc2626', marginTop: 10, lineHeight: 1.5 }}>
          ...
          <div style={{ border: '0.5px solid #e5e7eb', borderRadius: 16, padding: '16px 20px', background: '#fff' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                'Stay within the guide path — boundary alerts will warn you',
